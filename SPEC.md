...
## Add Northfield Cemetery — Funeral Processions, Gravedigging & After-Dark Robbery

**Landmark**: `CEMETERY` (already generated and registered in `LandmarkType`, no system exists yet)

### Overview

The cemetery on the edge of Northfield is walled, gloomy, and home to the town's recent dead.
It's a source of quiet reflection, illicit midnight income, and the odd funeral spectacle.
A `CemeterySystem` brings the CEMETERY landmark to life.

### Physical Layout (already generated by `WorldGenerator.generateCemetery`)

The existing generation creates a walled grass plot with gravestones. The system
builds gameplay on top of this without changing the world generator:

- **Gravestone props** (`GRAVESTONE_PROP`): each has a name and epitaph (procedurally generated
  from a pool of names and lines). Pressing **E** on a gravestone reads its inscription in the
  SpeechLogUI.
- **Grave plots** (`GRAVE_PLOT_PROP`): 6 diggable plots inside the cemetery. Each holds a
  buried `GRAVE_LOOT` cache (see Drop Table below). Plots start as undisturbed.
- **Groundskeeper's Shed** (`SHED_PROP`) near the gate: contains a `SPADE` item and a
  `GARDENING_GLOVES` item. The `SPADE` is required to dig graves.
- **Iron Gate** at the south entrance: open 08:00–20:00. Outside those hours, the player must
  use `BOLT_CUTTERS` (consuming 1 durability) or climb over (AGILITY skill Tier 2+).

### Key Characters

- **Vernon** (`GROUNDSKEEPER` NPCType): present 08:00–17:00. Patrols the cemetery paths.
  Passive unless the player attempts to dig a grave while he watches — then calls police
  (+2 Notoriety, `CrimeType.GRAVE_ROBBING`). If the player has donated to the Food Bank ≥ 3
  times, Vernon tips his cap and ignores one grave dig.
- **Funeral Director** (`FUNERAL_DIRECTOR` NPCType): spawns with a funeral procession (see below).
  Passive; delivers solemn speech lines. Never hostile.
- **Mourners** (`MOURNER` NPCType, 3–6 per funeral): follow the coffin, weep and murmur.
  Drop `CONDOLENCE_CARD` items on departure (used in crafting or as a social item).

### Funeral Procession Mechanic

Once per in-game day (random time 10:00–15:00), a funeral procession arrives:

1. A `COFFIN_PROP` spawns at the gate and slowly travels to a designated grave plot.
2. 3–6 `MOURNER` NPCs follow the coffin. Each mourner has a 30% chance of dropping flowers
   (`FLOWERS_PROP`) at the graveside before departing.
3. The `FUNERAL_DIRECTOR` NPC leads the procession and delivers a graveside speech (3 lines
   from a pool of 10 Victorian-tinged British non-denominational lines).
4. After 90 real seconds, mourners and the director despawn. The freshly covered grave plot
   becomes **disturbed** — it can be robbed more easily (4 hits with `SPADE` vs 8 for old plots).
5. A `FRESH_GRAVE` rumour is seeded in a nearby NPC ("Did you see the funeral today? Poor sod.").
6. **Player attendance**: If the player watches the full procession (within 6 blocks for ≥ 60
   seconds), they receive +2 Community Respect and a tooltip: "Paying your respects costs
   nothing."
7. **Disrupting the procession** (attacking a mourner or the director): +3 Notoriety,
   `CrimeType.DISTURBING_THE_PEACE`, `MOURNER` NPCs flee, director calls police.

### Gravedigging

- Player equips `SPADE` (found in shed or craftable: 2 WOOD + 1 SCRAP_METAL).
- Face a `GRAVE_PLOT_PROP` and **left-click** to dig. Each click is one hit.
  - Old grave: 8 hits to uncover.
  - Fresh (recently buried, < 1 in-game day): 4 hits to uncover.
- On completion, the plot replaces with `OPEN_GRAVE_PROP` and drops the loot cache.
- Vernon witnessing this triggers police call (see above).
- After 2 in-game hours, the open grave auto-fills back (plot returns to closed state, loot
  is gone if uncollected).

### Grave Loot Drop Table (`GRAVE_LOOT` cache, pick 2–4 items from):
...
| `WOOLLY_HAT` | 20 |
| `CIGARETTE` ×1–3 | 20 |
| `WEDDING_RING` (new item, fenceable for 8 COIN) | 10 |
| `POCKET_WATCH` (new item, fenceable for 12 COIN) | 8 |
| `PRESCRIPTION_MEDS` ×1 | 5 |
| `ANTIDEPRESSANTS` ×1 | 5 |
| `OLD_PHOTOGRAPH` (flavour item, unsellable; tooltip: "Someone loved this person.") | 12 |

### After-Dark Atmosphere

- Between 20:00–06:00, the cemetery gate is locked (BOLT_CUTTERS or AGILITY Tier 2 to enter).
- A single `LANTERN_PROP` flickers near the groundskeeper's shed.
- `BIRD` NPCs (owls) spawn inside the cemetery walls at night — passive, add ambient calls.
- Wandering `GHOST_FIGURE` visual effect (particle system only, no NPC): a translucent white
  `ParticleSystem` effect drifts slowly between gravestones 00:00–04:00. Non-interactive;
  purely atmospheric.

### New Items

- `SPADE` — tool; used to dig grave plots (8 uses before breaking).
- `WEDDING_RING` — loot item; sell to `FENCE` NPC or `PAWN_SHOP` for 8 COIN.
- `POCKET_WATCH` — loot item; sell to `FENCE` NPC or `PAWN_SHOP` for 12 COIN.
- `CONDOLENCE_CARD` — social item; giving one to a MOURNER NPC after the funeral raises
  Community Respect +1.
- `OLD_PHOTOGRAPH` — flavour item; not sellable; examining it displays: "Someone loved this person."

### New NPCTypes

- `GROUNDSKEEPER` — Vernon; cemetery warden, patrols, calls police on grave robbing.
- `FUNERAL_DIRECTOR` — leads processions; passive; solemn speech.
- `MOURNER` — follows coffin; drops flowers; flees on violence.

### New CrimeType

- `CriminalRecord.CrimeType.GRAVE_ROBBING` — +2 Notoriety per dig witnessed by Vernon.

### Achievements

- `GRAVE_ROBBER` — Successfully dig and loot a grave.
- `RESPECTFUL` — Attend a full funeral procession without disrupting it.
- `POCKET_FULL_OF_SORROW` — Sell a WEDDING_RING or POCKET_WATCH to the pawn shop.
- `NIGHT_OWL` — Enter the cemetery after midnight (inside CEMETERY boundary, hour ≥ 00:00).

### Integrations

- **FenceSystem** / **PawnShopSystem**: `WEDDING_RING` and `POCKET_WATCH` added to buy tables.
- **WitnessSystem**: if a POLICE NPC is within 10 blocks when the player digs, +1 Wanted star.
- **RumourNetwork**: `FRESH_GRAVE` rumour type seeded on each funeral; spreads via NPC gossip.
- **NoiseSystem**: each `SPADE` hit generates noise level 3 (audible to Vernon and nearby police).
- **WeatherSystem**: rain during a funeral procession adds +1 mourner NPC and +5 Community
  Respect for attendance (miserable British solidarity).
- **AchievementSystem**: four new achievements (above).

### Unit Tests

- Funeral procession spawns correct NPC count (3–6 mourners + director + coffin prop).
- Grave loot table produces 2–4 items drawn from the defined pool.
- Fresh grave dig takes 4 hits; old grave takes 8 hits.
- Vernon witness range check correctly triggers police call.
- Gate locked/unlocked based on hour and item possession.
- GHOST_FIGURE effect only spawns 00:00–04:00.

### Integration Tests — implement these exact scenarios:

1. **Funeral procession arrives and covers a grave**: Advance in-game time to 11:00. Trigger
   `CemeterySystem.triggerFuneral()`. Verify a `COFFIN_PROP` exists in the world within the
   CEMETERY boundary. Verify 3–6 `MOURNER` NPCs are present. Advance 90 real seconds (or
   equivalent simulation frames). Verify mourners and the director have despawned. Verify the
   target grave plot is in DISTURBED state. Verify a `FRESH_GRAVE` rumour exists in at least
   one NPC's rumour buffer within 30 blocks.

2. **Player digs fresh grave in 4 hits**: After the procession above, equip player with `SPADE`.
   Place player adjacent to the DISTURBED plot. Simulate 4 left-click hits on the plot.
   Verify the plot is now `OPEN_GRAVE_PROP`. Verify 2–4 items from the loot table have
   been spawned (SmallItem entities) at the plot position.

3. **Vernon calls police on witnessed dig**: Place `GROUNDSKEEPER` Vernon within 8 blocks of a
   grave plot. Equip player with `SPADE`. Simulate 1 dig hit. Verify `CrimeType.GRAVE_ROBBING`
   has been added to `CriminalRecord`. Verify `NotorietySystem` notoriety has increased by 2.
   Verify at least one nearby POLICE or PCSO NPC has been alerted (WantedSystem star ≥ 1).

4. **Player attendance rewards Community Respect**: Start with Community Respect = 20.
   Trigger a funeral. Place player within 6 blocks of the COFFIN_PROP position. Advance
   60 real seconds without moving away. Verify Community Respect has increased to 22.
   Verify the tooltip "Paying your respects costs nothing." has been triggered.

5. **Locked gate requires BOLT_CUTTERS after 20:00**: Set in-game time to 21:00. Place player
   at the CEMETERY gate. Verify that attempting to pass through the gate without BOLT_CUTTERS
   keeps the player outside (Z coordinate does not change). Give the player `BOLT_CUTTERS`.
   Press E on the gate. Verify the gate is now passable. Verify `BOLT_CUTTERS` durability
   has decreased by 1.
...
## Add Northfield Bingo Hall — Tuesday Afternoon Dabbers, Rigged Cards & Pensioner Solidarity

**Goal**: Add the Northfield Bingo Hall ("Lucky Stars Bingo") as a distinct landmark on the
high street — a loud, strip-lit social institution where pensioners spend their Tuesday
afternoons, and where the player can win coins, cheat with a rigged card, or cause chaos
by shouting "BINGO!" early.

### The Building

The Bingo Hall is generated by `WorldGenerator` as a single-storey brick building with a
garish `BUILDING_SIGN` reading "LUCKY STARS BINGO — Eyes Down 2pm". Interior contains:
- A rows of `BENCH_PROP` seats (player can sit)
- A `BINGO_CALLER_PODIUM_PROP` at the front (a lectern prop) where `CALLER` NPC stands
- A `PRIZE_BOARD_PROP` on one wall showing the current top prize
- A `REFRESHMENT_COUNTER_PROP` (selling TEA_CUP and BISCUIT items, 1 COIN each)

New `LandmarkType.BINGO_HALL` registered in `WorldGenerator`.

### BingoSystem — Core Mechanics

A new `BingoSystem` class manages all bingo state.

**Session schedule**: Sessions run Tuesday and Thursday in-game (days 2 and 4 of the
7-day week, detected from `TimeSystem`). Each session:
- Doors open at 13:30 (entry permitted), eyes down at 14:00.
- Closed 10:00–13:30 (locked doors) and after 17:00.
- Entry costs **2 COIN** per session (paid to the `CALLER` NPC via E interaction at the door).

**The card system**:
- On entry, the player receives a virtual bingo card (`BingoCard`) — a 5×5 grid of
  numbers (1–90), one free centre square.
- Each in-game minute the CALLER NPC announces a number (seeded from a deterministic
  shuffle of 1–90). The number appears as a `CALLER` NPC speech bubble ("Two fat ladies —
  eighty-eight!") using authentic British bingo calls for numbers 1–90 where available,
  plain number fallback otherwise.
- The player presses **E** on their card (displayed as a small HUD overlay at the
  bottom-left when inside the hall) to "dab" (mark) numbers manually. Auto-dab option
  available: hold **E** for 2 seconds to toggle auto-dabbing.
- **Winning**: completing a full row, column, or diagonal triggers "LINE!" (win 5 COIN).
  Completing the full card triggers "FULL HOUSE!" (win 15 COIN, plus trophy prop
  `BINGO_TROPHY_PROP` placed in player's nearest squat if they have one). A `WINNER`
  rumour is seeded into nearby NPCs.

**Pensioner NPCs**:
- 4–8 `PENSIONER` NPCs attend each session, seated at their usual spots.
- They auto-dab correctly (no need to help them).
- If a PENSIONER wins before the player, they shout "BINGO!" and the session ends.
  Players still in-progress receive a consolation prize: 1 COIN.
- If the player is rude (punches or shoves a PENSIONER inside), all remaining
  PENSIONERs stand up in solidarity, shout "Disgraceful!", and the player is ejected
  (teleported to the entrance). NeighbourhoodWatchSystem anger +10.

### Rigged Card Mechanic

The player can cheat by using a `RIGGED_BINGO_CARD` item (crafted: 1 BET_SLIP + 1 COIN
+ 2 WOOD via crafting menu — the card is clearly homemade). When a RIGGED_BINGO_CARD is
in the player's inventory on entry, it silently replaces their fair card with a card that
needs only 3 more numbers to complete (starts partially pre-dabbed).

- Cheat detection chance: 5% per number called while using a rigged card. If caught,
  the CALLER NPC announces "We've got a cheater — number 9!", all PENSIONERs boo, the
  player is ejected, Notoriety +8, CriminalRecord offence added.
- Tooltip on first rigged card use: "It's only cheating if you get caught."
- Tooltip on being caught: "You absolute melt."
- Achievement `EYES_DOWN` — win a full house legitimately (no rigged card).
- Achievement `CHEEKY_DABBER` — win using a rigged card without being caught.

### Shouting "BINGO!" Early

The player can press **B** during a session to shout "BINGO!" at any time — even if their
card is incomplete.

- If card IS complete: normal win. (Same as auto-detection above.)
- If card is NOT complete: CALLER checks ("Just bear with me, love — let me check that
  card"). After 3 seconds, announces "False call! Shame on you!" All NPCs laugh (speech
  bubbles). Player loses their entry fee (no refund). Session continues. Tooltip: "You
  absolute bellend."
- Cooldown: can only false-shout once per session.

### System Integrations

- **TimeSystem**: sessions tied to in-game day/time; doors locked outside session hours.
- **NotorietySystem**: cheating caught → +8 Notoriety; ejection → +3.
- **RumourNetwork**: `WINNER` rumour seeded when player wins full house.
- **NeighbourhoodWatchSystem**: attacking PENSIONERs → anger +10.
- **StreetEconomySystem**: BENEFIT_DAY raises participation (extra 2 PENSIONERs attend).
- **StreetSkillSystem**: TRADING Tier 2 perk lets the player peek at the next 3 called
  numbers on the prize board prop (flavour: "You've been in enough bingo halls to know
  when the caller hesitates").
- **WarmthSystem**: Bingo Hall interior is a warm shelter (+5 Warmth/min while seated).
- **AchievementSystem**: `EYES_DOWN`, `CHEEKY_DABBER`, `SOLIDARITY` (get ejected by
  pensioner uprising).

**New `Material` entries**: `RIGGED_BINGO_CARD`, `TEA_CUP`, `BISCUIT`, `BINGO_TROPHY`.
**New `NPCType` entry**: `CALLER` (bingo caller; stands at podium; announces numbers; ejects cheaters).

**Unit tests**: BingoCard generation (5×5, unique numbers 1–90, free centre), number calling
shuffle (no repeats), win detection (row/column/diagonal/full house), rigged card setup
(pre-dabbed correctly), cheat detection probability, false-BINGO penalty, pensioner ejection
trigger, session schedule (open only on correct days/times), warmth gain while seated.

**Integration tests — implement these exact scenarios:**

1. **Bingo Hall exists and is accessible at session time**: Generate the world. Verify a
   landmark of type `BINGO_HALL` exists. Set time to Tuesday 14:00. Give player 2 COIN.
   Place player at the bingo hall entrance. Press **E** on the CALLER NPC at the door.
   Verify 2 COIN deducted from inventory. Verify player receives a `BingoCard` (not null).
   Verify the HUD bingo card overlay is visible.

2. **Number is called each in-game minute**: Enter a session (2 COIN paid). Record
   `bingoSystem.getLastCalledNumber()` (should be -1 before first call). Advance simulation
   by 1 in-game minute. Verify `getLastCalledNumber()` is now in range [1, 90]. Verify
   the CALLER NPC's speech bubble is non-null and contains the number. Advance another
   minute. Verify a different number is called (no repeats within a session).

3. **LINE win pays 5 COIN**: Enter session. Set up the player's card so the first row
   needs exactly 1 more number. Force `bingoSystem.callNumber(n)` where n completes the
   row (auto-dab enabled). Verify `bingoSystem.isLineWon()` is true. Verify player
   inventory gains 5 COIN. Verify a `WINNER` rumour is seeded into at least one nearby NPC.

4. **FULL HOUSE win pays 15 COIN**: Enter session. Force-dab all numbers except one.
   Call the final number. Verify `bingoSystem.isFullHouseWon()` is true. Verify player
   gains 15 COIN. Verify a `BINGO_TROPHY_PROP` is placed in the player's squat (or at
   the player's position if no squat). Verify a `WINNER` rumour exists in the RumourNetwork.

5. **Pensioner wins before player — consolation prize**: Enter session. Simulate 60
   in-game minutes without the player completing a line. Verify at least one PENSIONER
   NPC shouted "BINGO!" (speech bubble recorded). Verify session state is ended. Verify
   player received exactly 1 COIN consolation prize.

6. **Rigged card reduces numbers needed**: Craft a `RIGGED_BINGO_CARD` (give player
   1 BET_SLIP + 1 COIN + 2 WOOD; craft). Verify item in inventory. Enter session with
   rigged card. Verify `bingoSystem.getPlayerCard().getDabbedCount()` is ≥ 20 (pre-dabbed
   card needs only a few more numbers). Advance until card completes. Verify player wins
   before any pensioner.

7. **Cheat detection ejects and adds Notoriety**: Set cheat detection to guaranteed
   (override probability to 1.0f for test). Enter session with RIGGED_BINGO_CARD. Call
   one number. Verify player is ejected (position is now outside the bingo hall AABB).
   Verify Notoriety increased by 8. Verify CriminalRecord has 1 new offence. Verify CALLER
   speech bubble contains "cheater".

8. **False BINGO triggers penalty**: Enter session. Do NOT complete any line. Press **B**.
   Verify CALLER speech contains "False call". Verify 2 COIN entry fee is lost (cannot
   be refunded). Verify session continues (CALLER keeps calling numbers). Press **B**
   again immediately. Verify second false shout is ignored (cooldown active, no second
   penalty speech).

9. **Pensioner solidarity ejection**: Enter session with 4 PENSIONER NPCs seated.
   Punch one PENSIONER. Verify all remaining PENSIONER NPCs switch to a reaction state
   (speech bubble "Disgraceful!"). Verify player is teleported to the entrance (outside
   the hall AABB). Verify `NeighbourhoodWatchSystem.getAnger()` increased by 10.

10. **Hall locked outside session hours**: Set time to Wednesday 14:00 (non-session day).
    Place player at bingo hall entrance. Press **E** on door. Verify player receives a
    "Closed — eyes down Tuesdays and Thursdays" response. Verify player cannot enter
